...
import { emptyProfile, Profile, ProfilePhoto, ProfileStatus } from "@/lib/types";
...
export default function MatchbookApp() {
  ...
  const deletePhoto = async (photo: ProfilePhoto) => { if (!selected || !confirm("Delete this photo? This cannot be undone.")) return; setBusy(true); try { await profileStore.removePhoto(photo); const next = { ...selected, photos: selected.photos.filter(p => p.id !== photo.id) }; if (profileStore.isDemo) await profileStore.save(next); setSelected(next); setProfiles(p => p.map(x => x.id === next.id ? next : x)); setNotice("Photo deleted"); } catch (e) { setNotice(e instanceof Error ? e.message : "Could not delete photo"); } finally { setBusy(false); } };
    ...
    <main>
      ...
      {view === "detail" && selected && <section className="page detail-page">
        ...
        {editing ? <ProfileForm profile={draft} setProfile={setDraft} title="Edit profile" subtitle={`Update ${selected.name}'s information.`} onSave={() => updateSelected(draft)} busy={busy} /> : <>
          ...
          <div className="detail-columns"><div className="detail-main"><InfoSection profile={selected} /><section className="panel photos"><div className="section-head"><div><h2>Photos</h2><p>{selected.photos.length ? `${selected.photos.length} saved` : "No photos added yet"}</p></div><label className="secondary upload-label"><ImagePlus /> Add photos<input type="file" multiple accept="image/jpeg,image/png,image/webp,image/heic" onChange={e => uploadPhotos(e.target.files)} /></label></div>{selected.photos.length ? <div className="photo-grid">{selected.photos.map(p => <div className="photo-item" key={p.id}><img src={p.url} alt={`${selected.name} photo`} /><button type="button" className="photo-remove" onClick={() => deletePhoto(p)} aria-label="Delete photo"><Trash2 /></button></div>)}</div> : <label className="photo-empty"><Camera /><strong>Add one or more photos</strong><span>JPG, PNG, WEBP or HEIC · up to 10 MB each</span><input type="file" multiple accept="image/jpeg,image/png,image/webp,image/heic" onChange={e => uploadPhotos(e.target.files)} /></label>}</section></div>
